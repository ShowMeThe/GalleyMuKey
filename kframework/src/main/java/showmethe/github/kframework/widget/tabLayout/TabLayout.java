...
package showmethe.github.kframework.widget.tabLayout;
...
public class TabLayout extends HorizontalScrollView {
    ...
    public TabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTabStrip.setSelectedIndicatorHeight(
                a.getDimensionPixelSize(R.styleable.TabLayout_tabIndicatorHeight, 10));
        ...
        try {
            mTabTextSize = ta.getDimensionPixelSize(
                    R.styleable.TextAppearance_android_textSize, 23);
        ...
        int tabIndicatorColor = a.getColor(R.styleable.TabLayout_tabIndicatorColor,ContextCompat.getColor(context,R.color.white));
        a.recycle();

        TypedArray ma = context.obtainStyledAttributes(attrs,R.styleable.TabLayout);

        int indicatorStartColor = ma.getColor(
                R.styleable.TabLayout_indicatorStartColor, tabIndicatorColor);
        int indicatorEndColor = ma.getColor(
                R.styleable.TabLayout_indicatorEndColor, tabIndicatorColor);
    ...
    private void ensureScrollAnimator() {
        if (mScrollAnimator == null) {
            ...
            mScrollAnimator.setInterpolator(AnimationUtils.LINEAR_OUT_SLOW_IN_INTERPOLATOR);
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        private void updateIndicatorPosition() {
            ...
            if (selectedTitle != null && selectedTitle.getWidth() > 0) {
                ...
                if (mSelectionOffset >= 0f && mSelectedPosition < getChildCount() - 1) {
                   ...
                   // float nowDis  = (right - left - mTabTextSize * selectedTitle.getTab().getText().length())/2;
                    float nowDis  = (right - left - selectedTitle.getTextView().getPaint().measureText(selectedTitle.getTab().getText().toString()))/2;
                   // float nextDis  = (nextTitle.getRight() - nextTitle.getLeft() - mTabTextSize * nextTitle.getTab().getText().length())/2;
                    float nextDis  = (nextTitle.getRight() - nextTitle.getLeft() -
                            nextTitle.getTextView().getPaint().measureText(nextTitle.getTab().getText().toString()))/2;


                    if (mSelectionOffset <= 0.5f) {
                        left = (int) (selectedTitle.getLeft() + nowDis);
                        right = (int) AnimationUtils.lerp(right-nowDis, nextTitle.getRight()- nextDis,
                                mSelectionOffset * 2);
                    } else {
                        left = (int) AnimationUtils.lerp(left+nowDis, nextTitle.getLeft()+nextDis,
                                (mSelectionOffset - 0.5f) * 2);
                        right = (int) (nextTitle.getRight() - nextDis);
                    }
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            TabView now = (TabView) getChildAt(mSelectedPosition);
            TabView next = (TabView) getChildAt(position);

            //float nowDis  = mIndicatorRight - mIndicatorLeft - mTabTextSize * now.getTab().getText().length();
            float nowDis  = mIndicatorRight - mIndicatorLeft -
                    now.getTextView().getPaint().measureText(now.getTab().getText().toString());
            //float nextDis  = targetView.getRight() - targetView.getLeft() - mTabTextSize * next.getTab().getText().length();
            float nextDis  = targetView.getRight() - targetView.getLeft() -
                    next.getTextView().getPaint().measureText(next.getTab().getText().toString());


            int targetLeft = (int) (targetView.getLeft() + nextDis/2);
            int targetRight = (int) (targetView.getRight() - nextDis/2);
            int startLeft;
            int startRight;

            startLeft = (int) (mIndicatorLeft + nowDis/2);
            startRight = (int) (mIndicatorRight - nowDis/2);
                ...
                animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator animator) {
                        ...
                        if (mSelectedPosition < position) {
                            if (fraction <= 0.5f) {
                                left = (int) (startLeft);
                                right = (int) AnimationUtils.lerp(startRight,
                                        targetRight, fraction * 2);
                            } else {
                                left = (int) AnimationUtils.lerp(startLeft,
                                        targetLeft, (fraction - 0.5f) * 2);
                                right = targetRight;
                            }
                        } else {
                            if (fraction <= 0.5f) {
                                left = (int) AnimationUtils.lerp(startLeft,
                                        targetLeft, fraction * 2);
                                right = startRight;
                            } else {
                                left = targetLeft;
                                right = (int) AnimationUtils.lerp(startRight,
                                        targetRight, (fraction - 0.5f) * 2);
                            }
                        }
                        setIndicatorPosition(left, right);
                    }
        ...
        public void draw(Canvas canvas) {
            ...
            if (mSelectedIndicatorPaint.getShader() == null || mIsColorDirty) {
                LinearGradient gradient = new LinearGradient(0, 0, right, 0, mStartColor, mEndColor, Shader.TileMode.CLAMP);
                mSelectedIndicatorPaint.setShader(gradient);
            }

                // visible rect
                mIndicatorRect.set(mIndicatorLeft , getHeight() - mSelectedIndicatorHeight,
