...
open class BaseItemAnimator : SimpleItemAnimator() {
    ...
    fun getRemoveDelay(holder: RecyclerView.ViewHolder): Long {
        return abs(holder.oldPosition * removeDuration / 4)
    }

    fun getAddDelay(holder: RecyclerView.ViewHolder): Long {
    ...
    open fun removeImp(holder: RecyclerView.ViewHolder) {
        val view = holder.itemView
        val animation = view.animate()
        animation.setDuration(removeDuration)
                .alpha(0f)
                .setListener(RemoveAnimatorListenerAdapter(animation,holder))
                .setStartDelay(getRemoveDelay(holder)).start()
    }

    open inner class RemoveAnimatorListenerAdapter(var animation: ViewPropertyAnimator, var holder: RecyclerView.ViewHolder) : AnimatorListenerAdapter() {
    ...
    open fun addImp(holder: RecyclerView.ViewHolder) {
        val view = holder.itemView
        val animation = view.animate()
        animation.alpha(1f).setDuration(addDuration)
                .setListener(AddAnimatorListenerAdapter(animation,holder)).setStartDelay(getAddDelay(holder)).start()
    }

    open inner class AddAnimatorListenerAdapter(var animation: ViewPropertyAnimator, var holder: RecyclerView.ViewHolder) : AnimatorListenerAdapter() {
        override fun onAnimationStart(animator: Animator) {
            dispatchRemoveStarting(holder)
        }
