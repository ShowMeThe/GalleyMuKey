...
private constructor(context: Context, private val mParentView: ViewGroup, private val mCallback: Callback) {
     ...
     */
    fun smoothSlideViewTo(child: View, finalLeft: Int, finalTop: Int, duration: Int): Boolean {
        capturedView = child
        activePointerId = INVALID_POINTER
     ...
     * [.continueSettling] calls
     */
    private fun forceSettleCapturedViewAt(finalLeft: Int, finalTop: Int, xvel: Int, yvel: Int, duration: Int): Boolean {
        var dura = duration
        ...
        if (dura == 0)
            dura = computeSettleDuration(capturedView!!, dx, dy, xvel, yvel)
    ...
    protected fun canScroll(v: View, checkV: Boolean, dx: Int, dy: Int, x: Int, y: Int): Boolean {
        ...
        return checkV && (v.canScrollHorizontally(-dx) || v.canScrollVertically(
            -dy
        ))
    }
    ...
    private fun getEdgeTouched(x: Int, y: Int): Int {
        ...
        if (x < mParentView.getLeft() + edgeSize)
            result = result or EDGE_LEFT
        if (y < mParentView.getTop() + edgeSize)
            result = result or EDGE_TOP
        if (x > mParentView.getRight() - edgeSize)
            result = result or EDGE_RIGHT
        if (y > mParentView.getBottom() - edgeSize)
            result = result or EDGE_BOTTOM
